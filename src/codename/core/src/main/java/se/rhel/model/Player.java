...
public class Player extends DynamicEntity {
    ...
    private void createPyshicsBody() {
        ...
        mWorld.addToWorld(playerShape,
                playerInfo,
                playerMotionState,
                new ModelInstance(Bodybuilder.INSTANCE.createCapsule(mPlayersize.x, mPlayersize.y), getPosition()),
                mBody);
    }
    ...
    private void checkOnGround() {
        mOnGround = false;
        from.set(getPosition());
        to.set(new Vector3(from.x, from.y - 1f, from.z));
